...
#
class JwtService
  SECRET_KEY = Rails.application.credentials.secret_key_base || ENV.fetch("SECRET_KEY_BASE", "dev-secret-key-change-in-production")
  ALGORITHM  = "HS256"
  ...
  def self.decode(token, expected_type: "access")
    ...
    if payload["type"] != expected_type
      raise DecodeError, "Invalid token type: expected #{expected_type}"
    end
